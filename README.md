...
| `test_app.py` | `python test_app.py`. Verifica que la predicción no dependa del orden, que la confianza salga del tramo correcto, que la cartelera, Betano y The Odds API se parseen bien, que el archivado y el ledger congelen y evalúen (dedup, resultado, CLV), que los homónimos avisen, que reemplazo y peso no dado bajen al que los tiene sin romper la simetría, que el método sea coherente por división, y que la app renderice con y sin cuotas. No toca la red. | — |
